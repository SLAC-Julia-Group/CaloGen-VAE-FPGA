...
parser.add_argument(
    "--input_file", "-i", help="Name of the input file to be evaluated."
)
parser.add_argument(
    "--reference_file",
    "-r",
...
if __name__ == "__main__":
    ...
    hlf = HLF.HighLevelFeatures(
        particle,
        filename="binning_dataset_{}.xml".format(args.dataset.replace("-", "_")),
    )
    ...
    if args.mode in ["all", "avg"]:
        ...
        plot_layer_comparison(
            hlf,
            shower.mean(axis=0, keepdims=True),
            reference_hlf,
    ...
    if args.mode in ["all", "avg-E"]:
        ...
        if "1" in args.dataset:
            ...
            for i in range(3, 7):
                plot_title.append(
                    "shower average for E in [{}, {}] MeV".format(
                        10**i, 10 ** (i + 1)
                    )
                )
    ...
    if args.mode in ["all", "cls-low", "cls-high", "cls-low-normed"]:
        ...
        if args.mode in ["all", "cls-low"]:
            source_array = prepare_low_data_for_classifier(
                source_file, hlf, 0.0, normed=False
            )
            reference_array = prepare_low_data_for_classifier(
                reference_file, reference_hlf, 1.0, normed=False
            )
        elif args.mode in ["cls-low-normed"]:
            source_array = prepare_low_data_for_classifier(
                source_file, hlf, 0.0, normed=True
            )
            reference_array = prepare_low_data_for_classifier(
                reference_file, reference_hlf, 1.0, normed=True
            )
        elif args.mode in ["cls-high"]:
            source_array = prepare_high_data_for_classifier(source_file, hlf, 0.0)
            reference_array = prepare_high_data_for_classifier(
                reference_file, reference_hlf, 1.0
            )
