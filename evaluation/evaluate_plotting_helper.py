...
def plot_Etot_Einc_discrete(hlf_class, reference_class, arg):
    ...
    for i in range(len(target_energies) - 1):
        ...
        which_showers_ref = (
            (reference_class.Einc.squeeze() >= target_energies[i])
            & (reference_class.Einc.squeeze() < target_energies[i + 1])
        ).squeeze()
        which_showers_hlf = (
            (hlf_class.Einc.squeeze() >= target_energies[i])
            & (hlf_class.Einc.squeeze() < target_energies[i + 1])
        ).squeeze()
    ...
    ax.axis("off")
    filename = os.path.join(
        arg.output_dir, "Etot_Einc_dataset_{}_E_i.png".format(arg.dataset)
    )
    plt.savefig(filename, dpi=300)
    plt.close()
...
def plot_Etot_Einc(hlf_class, reference_class, arg):
    ...
    if arg.mode in ["all", "hist-p", "hist"]:
        filename = os.path.join(
            arg.output_dir, "Etot_Einc_dataset_{}.png".format(arg.dataset)
        )
        plt.savefig(filename, dpi=300)
...
def plot_E_layers(hlf_class, reference_class, arg):
    ...
    for key in hlf_class.GetElayers().keys():
        ...
        if arg.mode in ["all", "hist-p", "hist"]:
            filename = os.path.join(
                arg.output_dir, "E_layer_{}_dataset_{}.png".format(key, arg.dataset)
            )
            plt.savefig(filename, dpi=300)
        if arg.mode in ["all", "hist-chi", "hist"]:
            seps = _separation_power(counts_ref, counts_data, bins)
...
def plot_ECEtas(hlf_class, reference_class, arg):
    ...
    for key in hlf_class.GetECEtas().keys():
        ...
        if arg.mode in ["all", "hist-p", "hist"]:
            filename = os.path.join(
                arg.output_dir, "ECEta_layer_{}_dataset_{}.png".format(key, arg.dataset)
            )
            plt.savefig(filename, dpi=300)
        if arg.mode in ["all", "hist-chi", "hist"]:
            seps = _separation_power(counts_ref, counts_data, bins)
...
def plot_ECPhis(hlf_class, reference_class, arg):
    ...
    for key in hlf_class.GetECPhis().keys():
        ...
        if arg.mode in ["all", "hist-p", "hist"]:
            filename = os.path.join(
                arg.output_dir, "ECPhi_layer_{}_dataset_{}.png".format(key, arg.dataset)
            )
            plt.savefig(filename, dpi=300)
        if arg.mode in ["all", "hist-chi", "hist"]:
            seps = _separation_power(counts_ref, counts_data, bins)
...
def plot_ECWidthEtas(hlf_class, reference_class, arg):
    ...
    for key in hlf_class.GetWidthEtas().keys():
        ...
        if arg.mode in ["all", "hist-p", "hist"]:
            filename = os.path.join(
                arg.output_dir,
                "WidthEta_layer_{}_dataset_{}.png".format(key, arg.dataset),
            )
            plt.savefig(filename, dpi=300)
        if arg.mode in ["all", "hist-chi", "hist"]:
            seps = _separation_power(counts_ref, counts_data, bins)
...
def plot_ECWidthPhis(hlf_class, reference_class, arg):
    ...
    for key in hlf_class.GetWidthPhis().keys():
        ...
        if arg.mode in ["all", "hist-p", "hist"]:
            filename = os.path.join(
                arg.output_dir,
                "WidthPhi_layer_{}_dataset_{}.png".format(key, arg.dataset),
            )
            plt.savefig(filename, dpi=300)
        if arg.mode in ["all", "hist-chi", "hist"]:
            seps = _separation_power(counts_ref, counts_data, bins)
...
def plot_cell_dist(shower_arr, ref_shower_arr, arg):
    ...
    if arg.mode in ["all", "hist-p", "hist"]:
        filename = os.path.join(
            arg.output_dir, "voxel_energy_dataset_{}.png".format(arg.dataset)
        )
        plt.savefig(filename, dpi=300)
    if arg.mode in ["all", "hist-chi", "hist"]:
        seps = _separation_power(counts_ref, counts_data, bins)
